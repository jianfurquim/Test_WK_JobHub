...
class CustomUserManager(BaseUserManager):
    def _create_user(self, cpf, email, password, **extra_fields):
        ...
        user = self.model(cpf=cpf, email=email, **extra_fields) # Create the user, passing cpf directly
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    email = models.EmailField(unique=True, null=True, blank=True, verbose_name='Email') # Added email, common for superusers
    ...
    USERNAME_FIELD = 'cpf'
    REQUIRED_FIELDS = ['name'] # Add 'email' here if you want it to be mandatory during createsuperuser
